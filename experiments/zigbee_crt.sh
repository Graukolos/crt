...
cargo run -r -- ZigBee/src/multitoken_tx/Top_ZigBee_tx.xdf ZigBee/src --out /tmp/zigbee --backend naive
cargo build --release --manifest-path /tmp/zigbee/Cargo.toml
hyperfine --warmup 3 '/tmp/zigbee/target/release/top_zigbee_tx -i /tmp/zigbee/big.in -w /tmp/zigbee/out.txt'
...
hyperfine --warmup 3 '/tmp/zigbee/Top_ZigBee_tx_cpp -i /tmp/zigbee/big.in -w /tmp/zigbee/out.txt'
